...
export default function DetalhesDaMoeda() {
    ...
    const [opcaoPeriodoGrafico, setOpcaoPeriodoGrafico] = useState(false);

    useEffect(() => {
        fetch(`https://api.coinstats.app/public/v1/charts?period=${periodoDoGrafico}&coinId=${params.id}`)
            .then((resp) => resp.json())
            .then((dados) => {
        ...
        <section className={styles.moedaDetalhada}>
            <div className={styles.cabecalho}>
                ...
                <div>
                    <h1>
                        {moeda.name}
                        <button onClick={() => adicionarFavorito(moeda)}>{!ehFavorito ? <AiOutlineStar size={25} color="#c6cfd2" /> : <AiFillStar size={25} color="#8b80db" />}</button>
                    </h1>
                    <p>{moeda.symbol}</p>
                </div>
            </div>
            ...
            <section className={styles.info}>
                <div className={styles.info__priceStats}>
                    ...
                    <div>
                        ...
                        <p>
                            1H{" "}
                            <span style={Number(moeda.priceChange1h) < 0 ? { color: "#b9283d" } : { color: "#66be54" }}>
                                {Number(moeda.priceChange1h) < 0 ? <RxTriangleDown /> : <RxTriangleUp />}
                                {moeda.priceChange1h}%
                            </span>
                        </p>
                        ...
                        <p>
                            1D{" "}
                            <span style={Number(moeda.priceChange1d) < 0 ? { color: "#b9283d" } : { color: "#66be54" }}>
                                {Number(moeda.priceChange1d) < 0 ? <RxTriangleDown /> : <RxTriangleUp />}
                                {moeda.priceChange1d}%
                            </span>
                        </p>
                        ...
                        <p>
                            1W{" "}
                            <span style={Number(moeda.priceChange1w) < 0 ? { color: "#b9283d" } : { color: "#66be54" }}>
